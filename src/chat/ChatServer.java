...
import chat.Member.ExistMember;
...
//		.forEach(socketClient->socketClient.send(json));
	...
	public void checkIdPass(SocketClient sender, String uid, String pwd)
	{
		//id key pass value json 파일에서 검색
		boolean isRightInfo = false;
		
		HashMap<String,String> infoDb = memberRepository.getLoginInfo(uid);
		
		if(pwd.equals(infoDb.get(uid))) 
			isRightInfo = true;
	...
	public synchronized void registerMember(JSONObject jsonObject) {
	
		Member member = new Member("hong", "1234");
		try {
			memberRepository.insertMember(member);
		} catch (ExistMember e1) {
			e1.printStackTrace();
		}
	}
	//메소드: 서버 종료
